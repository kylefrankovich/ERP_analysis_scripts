...
%%%%%%% plot w/ occipital waveforms (correct responses): 
...
%%%%%%% plot w/ occipital waveforms (all responses): 


% plot blocked:

% load ERP:
ERP = pop_loaderp( 'filename', [cur_sub '_LOTP_4AFC_ERP2_ERP_contra_ipsi_diff_blocked_mixed_occip_all_resps.erp'], 'filepath', sub_folder );
...
% (verified on 8/2/16 that adding occipital site does not 
% interfere w/ other electrode locations):

fprintf('\n******\nProcessing subject %s\n******\n\n', sub_to_process);

% load ERP (correct responses):
% ERP = pop_loaderp( 'filename', [cur_sub '_LOTP_4AFC_ERP2_ERP_contra_ipsi_diff_blocked_mixed.erp'], 'filepath', sub_folder );

% load ERP (all responses)
ERP = pop_loaderp( 'filename', [cur_sub '_LOTP_4AFC_ERP2_ERP_contra_ipsi_diff_blocked_mixed_all_resps.erp'], 'filepath', sub_folder );
